...
byte bn_ext(bn_t *number, size_t bytes);				/* Extend number size by n bytes */
byte bn_free(bn_t *number);					/* Free number */
...
byte bn_cpy(bn_t *result, bn_t *number);				/* Copy number into result reallocating the size */
byte bn_hcpy(bn_t *result, bn_t *number);				/* Copy number into result without reallocating (set to 0 the surplus) */
byte bn_ncpy(bn_t *result, bn_t *number, size_t size);		/* Copy n bytes from number to result starting from the less valuable byte */
byte bn_rev(bn_t *number);					/* Revers byte order */
